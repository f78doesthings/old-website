// All of this code may be remade in the future
...
function switchTheme(newTheme = theme) {
  try {
    if (theme === "custom") $(`#theme-custom`).attr("disabled", true);
      ...
      $(`link[href*="/assets/css/themes/${theme}.css"]`).attr("disabled", true);
    if (newTheme === "custom") $("link#theme-custom").attr("disabled", false);
    else
      $(`link[href*="/assets/css/themes/${newTheme}.css"]`).attr(
        "disabled",
        false
      );
...
$("#-pref-appearance-theme").change(() =>
  switchTheme($("#-pref-appearance-theme").val())
);
$("#-pref-appearance-url").change(() => switchTheme("custom"));
$("#-pref-appearance-reset").click(() => {
  reset++;
  if (reset >= 2) {
    $("#-pref-appearance-reset")
      .html(`<div class="preloader-wrapper small active" style="width: 24px; height: 24px; top: 6px;">
